...
Contract v1 is implemented by `receipt_extractor.evaluation` and exposed through
the separate `receipt-evaluator` entry point. The repository generator creates
the [eight synthetic inputs](../demo/evaluation-inputs), builds the exact
[content-addressed suite](../demo/evaluation-suite.json), captures the actual
[JSON report](../demo/evidence/evaluation-receipt.json) and
[text report](../demo/evidence/evaluation-receipt.txt), then runs and records
[full verification](../demo/evidence/evaluation-verification.json). The same
generator derives the reader-facing metrics and diagrams only after reloading
and validating the tracked report.

`make demo-check` repeats that complete workflow under a temporary output root
and requires every generated byte to match the tracked evidence. Tests also
re-execute all three evaluator commands with the API key absent, provider import
poisoned, and socket creation blocked.
